...
Used by RAG service to parse and chunk knowledge base documents
"""
...
def chunk_markdown(filepath: str, source_name: str, chunk_size: int = 400, overlap: int = 50) -> List[Dict]:
    """Split markdown into overlapping paragraph-level chunks with header context."""
...
def chunk_shipping(filepath: str) -> List[Dict]:
    ...
    # Domestic options
    for method, info in data.get("domestic", {}).items():
        text = (
    ...
    for region, info in data.get("international", {}).items():
        text = (
...
def load_all_chunks(kb_dir: str) -> List[Dict]:
    ...
    prod_file = kb / "products.json"
    if prod_file.exists():
        all_chunks.extend(chunk_products(str(prod_file)))

    policy_file = kb / "policies.md"
    if policy_file.exists():
        all_chunks.extend(chunk_markdown(str(policy_file), "policies.md"))

    faq_file = kb / "faqs.md"
    if faq_file.exists():
        all_chunks.extend(chunk_faq(str(faq_file)))

    shipping_file = kb / "shipping_zones.json"
    if shipping_file.exists():
        all_chunks.extend(chunk_shipping(str(shipping_file)))
